...
watchEffect(async () => {
  store.getRecette({ slug: route.params.slug });
});
</script>
<template>
  <form
    @submit.prevent="store.updateRecette({ slug: route.params.slug })"
    novalidate
  >
    <div class="flex flex-col mx-auto md:w-96 w-full">
      <h1 class="text-2xl font-bold mb-4 text-center">editer la recette</h1>
      <div class="flex flex-col gap-2 mb-4">
        <label for="plate_number" class="required">Nom de la recette</label>
        <input
